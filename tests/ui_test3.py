import sys
from PyQt6.QtWidgets import (
    QApplication, QMainWindow, QWidget, QPushButton, QLabel,
    QVBoxLayout, QHBoxLayout, QStackedWidget, QFrame
)
from PyQt6.QtCore import Qt, QPropertyAnimation, QRect


class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()

        self.setWindowTitle("Overlay Sidebar Example")
        self.setGeometry(200, 200, 800, 500)

        # --- Central Area with stacked pages ---
        central_widget = QWidget()
        self.setCentralWidget(central_widget)
        self.stack = QStackedWidget()
        self.stack.addWidget(self.create_page("🏠 Driver's Standing"))
        self.stack.addWidget(self.create_page("📝 Constructor's standing"))
        self.stack.addWidget(self.create_page("📒 Schedule "))

        # Hamburger button on top bar
        self.hamburger = QPushButton("☰")
        self.hamburger.setFixedSize(40, 40)
        self.hamburger.setStyleSheet("""
            QPushButton {
                background: #5a5aff;
                color: white;
                font-size: 18px;
                border-radius: 6px;
            }
            QPushButton:hover {
                background: #7878ff;
            }
        """)
        self.hamburger.clicked.connect(self.toggle_sidebar)

        top_bar = QHBoxLayout()
        top_bar.addWidget(self.hamburger, alignment=Qt.AlignmentFlag.AlignLeft)
        top_bar.addStretch()

        main_layout = QVBoxLayout(central_widget)
        main_layout.addLayout(top_bar)
        main_layout.addWidget(self.stack)

        # --- Sidebar (floating overlay) ---
        self.sidebar = QFrame(self)
        self.sidebar.setGeometry(-200, 0, 200, self.height())
        self.sidebar.setStyleSheet("background-color: #2e2e3e; color: white;")

        sidebar_layout = QVBoxLayout(self.sidebar)
        btn_home = QPushButton("🏠 Driver's Standing")
        btn_form = QPushButton("📝 Constructor's standing")
        btn_notes = QPushButton("📒 Schedule")

        for i, btn in enumerate((btn_home, btn_form, btn_notes)):
            btn.setStyleSheet("""
                QPushButton {
                    background: #3a3a4d;
                    color: white;
                    padding: 10px;
                    border-radius: 6px;
                }
                QPushButton:hover {
                    background: #5a5aff;
                }
            """)
            sidebar_layout.addWidget(btn)
            btn.clicked.connect(lambda _, idx=i: self.switch_page(idx))

        sidebar_layout.addStretch()

        # Animation object for smooth sliding
        self.anim = QPropertyAnimation(self.sidebar, b"geometry")
        self.anim.setDuration(300)

        self.sidebar_open = False

        # Dark mode
        self.setStyleSheet("QMainWindow { background-color: #1e1e2f; } QLabel { color: white; font-size: 16px; }")

    def toggle_sidebar(self):
        if self.sidebar_open:
            # Hide (slide left)
            self.anim.setStartValue(QRect(0, 0, 200, self.height()))
            self.anim.setEndValue(QRect(-200, 0, 200, self.height()))
        else:
            # Show (slide right)
            self.anim.setStartValue(QRect(-200, 0, 200, self.height()))
            self.anim.setEndValue(QRect(0, 0, 200, self.height()))

        self.anim.start()
        self.sidebar_open = not self.sidebar_open

    def switch_page(self, index):
        self.stack.setCurrentIndex(index)
        self.toggle_sidebar()  # auto-close after navigation

    def create_page(self, text):
        page = QWidget()
        layout = QVBoxLayout(page)
        label = QLabel(text)
        label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(label)
        return page


if __name__ == "__main__":
    app = QApplication(sys.argv)
    window = MainWindow()
    window.show()
    sys.exit(app.exec())
